...
trait SendMail {
      ...
      public function codorderplacedMailstore($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name, $store_n,$store_email) {
       ...
       $app_name = $logo->name;
        ...
        $data = array('to' => $store_email, 'from' => 'noreply@gogrocer.in', 'to-name'=>$store_n, 'from-name' => $app_name);

        Mail::send('admin.mail.codorderplacedstore', compact('cart_id','prod_name','price2','delivery_date','time_slot','user_email','user_name', 'store_n'), function ($m) use ($data){
                $m->from($data['from'], $data['from-name']);
                $m->to($data['to'], $data['to-name'])->subject("Order Successfully Placed");
            });
      ...
      public function codorderplacedMailadmin($cart_id,$prod_name,$price2,$delivery_date,$time_slot,$user_email,$user_name, $store_n,$admin_email,$admin_name) {
       ...
       $app_name = $logo->name;
        ...
        $data = array('to' => $admin_email, 'from' => 'noreply@gogrocer.in', 'to-name'=>$admin_name, 'from-name' => $app_name);

        Mail::send('admin.mail.codorderplacedadmin', compact('cart_id','prod_name','price2','delivery_date','time_slot','user_email','user_name', 'store_n'), function ($m) use ($data){
                $m->from($data['from'], $data['from-name']);
                $m->to($data['to'], $data['to-name'])->subject("Order Successfully Placed");
            });
     ...
     public function coddeloutMailstore($cart_id, $prod_name, $price2, $user_email, $user_name, $rem_price,$store_n, $store_email) {
       ...
       $app_name = $logo->name;
        ...
        Mail::send('admin.mail.coddel_outstore', compact('cart_id', 'prod_name', 'price2','currency','rem_price'), function ($m) use ($data){
                $m->from($data['from'], $data['from-name']);
                $m->to($data['to'], $data['to-name'])->subject("Out For Delivery");
            });
     ...
     public function coddeloutMailadmin($cart_id, $prod_name, $price2, $user_email, $user_name, $rem_price,$admin_name,$admin_email) {
       ...
       $app_name = $logo->name;
        ...
        Mail::send('admin.mail.coddel_outadmin', compact('cart_id', 'prod_name', 'price2','currency','rem_price'), function ($m) use ($data){
                $m->from($data['from'], $data['from-name']);
                $m->to($data['to'], $data['to-name'])->subject("Out For Delivery");
            });
    ...
    public function deloutMail($cart_id, $prod_name, $price2,$user_email, $user_name,$rem_price) {
     ...
     public function rechargeMail($user_id,$user_name, $user_email, $user_phone,$add_to_wallet) {
       ...
       $currency = DB::table('currency')
        ...
        Mail::send('admin.mail.recharge', compact('add_to_wallet','user_name','user_id','currency','add_to_wallet', 'app_name'), function ($m) use ($data){
                $m->from($data['from'], $data['from-name']);
     ...
     public function sendrejectmail($cause,$user,$cart_id) {
       ...
       $currency = DB::table('currency')
        ...
        $data = array('to' => $user->email, 'from' => 'noreply@gogrocer.in', 'to-name'=>$user->name, 'from-name' => $app_name);
...
$link = config('base_url') . 'password/reset/' . $token . '?email=' . urlencode($email);
    ...
    $data = array('to' => $email, 'from' => 'noreply@gogrocer.in', 'to-name'=>$email, 'from-name' =>$app_name);
