...
class InstallController extends Controller
{
    ...
    public function verifyLicense(Request $request)
    {
        ...
        if($license){
            $verify  = $licenseApi->verify_license(false, $license->license, $license->client);
            $status = $verify["status"] ? "active" : "inactive";

            $now = \Carbon\Carbon::now();
            DB::update('update licensebox set status = ?, updated_at=now() where id = ?',[$status,$license->id]);                   

            if($status == "inactive"){
                return view('installer.verifylicense');
            }
