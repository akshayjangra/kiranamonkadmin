...
class PayoutController extends Controller
{
    ...
    public function pay_req(Request $request)
    {
         ...
         $total_earnings=DB::table('payout_requests')
                            ->join('store', 'payout_requests.store_id', '=', 'store.id')
                           ...
                           ->select('store.id','store.store_name', 'store.phone_number','store.address','store.email','store_earning.paid','payout_requests.payout_amt', 'payout_requests.complete','payout_requests.req_id','store_bank.ac_no', 'store_bank.ifsc','store_bank.holder_name','store_bank.bank_name', 'store_bank.upi', DB::raw('SUM(orders.total_price)-SUM(orders.total_price)*(store.admin_share)/100 as sumprice'))
     ...
     public function store_pay(Request $request)
    {
        ...
        if($update){
            ...
            $welcomeMail = $this->payoutMail($amt,$store_name,$user_email,$app_name); 
             ...
             return redirect()->back()->withSuccess(trans('keywords.Amount of').' '.$amt.' '.trans('keywords.marked paid successfully against your request'));
        }
        else{
